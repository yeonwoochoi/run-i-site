<template>
  <section ref='aboutSection' class='relative w-screen h-screen flex lg:flex-row flex-col items-center py-[80px]'>
    ...
    <div class='flex flex-1 justify-center items-center gap-8 w-full max-w-[80%] max-h-[150px] pt-[100px] overflow-hidden lg:hidden block'>
      <div ref='aboutMobileTab' class='flex space-x-4 overflow-x-auto scroll-smooth scrollbar-hide'>
        <div
    ...
    <div class="flex-1 flex flex-col justify-center items-center lg:px-0 px-10">
      ...
      <div class="lg:w-3/5 w-full mt-5">
        <p ref="aboutCopy" class="text-base md:text-lg font-normal leading-7"></p>
      </div>
    </div>
...
const aboutCopy = ref(null)
const currentCount = ref(null)
const currentIndex = ref(0)
const aboutMobileTab = ref(null);
...
onMounted(() => {
  ...
  const stickyHeight = window.innerHeight * aboutTitles.length
  const serviceHeight = 38
  const imgHeight = 250

  aboutCopy.value.textContent = aboutContents[0][0]
  let currentSplitText = new SplitType(aboutCopy.value, { types: 'lines' })
  ...
  const serviceWidths = aboutTitles.map((service) => {
    measureContainer.textContent = service
    return measureContainer.offsetWidth + aboutTitles.length
  })
    ...
    return new Promise((resolve) => {
      gsap.to(currentSplitText.lines, {
        ...
        onComplete: () => {
          ...
          aboutCopy.value.textContent = aboutContents[index][0]
          currentSplitText = new SplitType(aboutCopy.value, { types: 'lines' })
